...
static uint16_t endwid, endhgt;
static uint16_t bplanes;
static unsigned long skipv;
static unsigned long planelengthw;
...
/* ---------------------------------------------------------------- */
/* ---------------------------------------------------------------- */
static void orderedto1(SMURF_PIC *picture, uint8_t *ziel)
{
	uint8_t *buffer = 0;
	uint8_t *line;
	uint8_t *pal;
	const uint8_t *matline;
	uint8_t *pixbuf;
	uint8_t bit;
	uint8_t v;
	uint8_t depth;
	...
	if (depth == 16)
		buffer16 = (uint16_t *) picture->pic_data;
	else
	...
	w = (width + 7) >> 3;
	planelengthr = (unsigned long) w * height;	/* LÑnge einer Plane in Bytes */
	...
	memset(ziel, 0, ((unsigned long) endwid * endhgt) >> 3);
	...
	if (picture->format_type == FORM_STANDARD)
	{
		...
		do
		{
			...
			do
			{
				val = *line++;
				pal = palette + val * 3;
				if ((((unsigned long) pal[0] * 872L + (unsigned long) pal[1] * 2930L + (unsigned long) pal[2] * 296L) >> 12) <= matline[x & 15])
					*ziel |= 0x01 << bit;
	...
	else
	{
		...
		do
		{
			...
			} else if (depth == 24)
			{
				...
				do
				{
					if ((((unsigned long) buffer[0] * 872L + (unsigned long) buffer[1] * 2930L + (unsigned long) buffer[2] * 296L) >> 12) <= matline[x & 15])
						*ziel |= 0x01 << bit;
			...
			} else
			{
				...
				{
					val = *buffer16++;
					if ((((unsigned long) ((val & 0xf800) >> 8) * 872L + (unsigned long) ((val & 0x7e0) >> 3) * 2930L + (unsigned long) ((val & 0x1f) << 3) * 296L) >> 12) <= matline[x & 15])
						*ziel |= 0x01 << bit;
...
static void make_pal(SMURF_PIC *picture, uint8_t *convtab)
{
	picture->red[0] = 255;
	picture->grn[0] = 255;
	picture->blu[0] = 255;
...
/* ---------------------------------------------------------------- */
/* ---------------------------------------------------------------- */
static void orderedto4(SMURF_PIC *picture, uint8_t *ziel)
{
	uint8_t *buffer = 0;
	uint8_t *line;
	uint8_t *pal;
...
static uint16_t make_tabs(SMURF_PIC *picture, uint8_t *divN, uint8_t *divN6, uint8_t *divN6_1, uint8_t *divN36, uint8_t *divN36_1, short *modr, short *modg, short *modb)
{
	uint8_t r, g, b;
	uint8_t rdiv, gdiv, bdiv;
	uint16_t levels;
	uint16_t offset;
	uint16_t i;
	uint16_t index;
	...
	for (r = 0; r < rdiv; r++)
		for (g = 0; g < gdiv; g++)
			for (b = 0; b < bdiv; b++)
			{
				...
				picture->red[index] = (short)((long) r * 255L / (rdiv - 1));
				picture->grn[index] = (short)((long) g * 255L / (gdiv - 1));
				picture->blu[index] = (short)((long) b * 255L / (bdiv - 1));
			}
...
/*------------------------------------------------------------------*/
/*------------------------------------------------------------------*/
static void f_dither_orderedcol(SMURF_PIC *picture, uint8_t *ziel, DITHER_DATA *dither)
{
	uint8_t *buffer = 0;
	uint8_t *line;
	uint8_t *pal;
	...
	uint8_t divN6_1[256];
	uint8_t divN36[256];
	uint8_t divN36_1[256];

	short modr[256], modg[256], modb[256];
	uint16_t *buffer16 = 0;
	uint16_t x, y;
	uint16_t ind;
	uint16_t bh, bl;
	...
	} else
	{
		if (picture->format_type == FORM_PIXELPAK)
			skipBytes = (width - dither->w) * (depth >> 3);
		else
			skipBytes = 0;
	}
	...
	if (picture->format_type == FORM_PIXELPAK)
	{
		buffer += ((long) dither->x * (long) (depth >> 3)) + ((long) dither->y * picture->pic_width * (depth >> 3));
		buffer16 += ((long) dither->x * (long) (depth >> 3)) + ((long) dither->y * picture->pic_width * (depth >> 3));
	} else
	{
		buffer += (dither->x >> 3) + ((long) dither->y * ((width + 7) >> 3));
	}
...
short dither_module_main(DITHER_DATA *dither)
{
	...
	if (picture->depth == 24)
	{
		...
		skipv = (unsigned long) picture->zoom * width * 3L;
	...
	} else
	{
		...
		skiph = picture->zoom;
		if (picture->format_type == FORM_PIXELPAK)
			skipv = (unsigned long) picture->zoom * width;
		else
			skipv = (unsigned long) picture->zoom * ((width + 7) >> 3);
		/* Unter den Tisch gefallene Bytes nachholen */
