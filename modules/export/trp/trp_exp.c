...
EXPORT_PIC *exp_module_main(GARGAMEL *smurf_struct)
{
	...
	*ziel2;
	...
	case MEXEC:
		...
		if (main[ENC_BINARY].ob_state & OS_SELECTED)
			file_config.encoding |= BINARY;
		if (main[INCLUDE_HEADER].ob_state & OS_SELECTED)
			file_config.include_header = 1;
	...
	case MMORE:
		...
		break;
			...
			/*---------------- Konfig canceln ----------------*/
	case MMORECANC:
		memcpy(&file_config, &dft_opts, sizeof(CONFIG));
