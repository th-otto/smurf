...
static void WriteTable(FILE * fd, const TIFFFaxTabEnt * T, int Size, const char *name)
{
	...
	fprintf(fd, "%s TIFFFaxTabEnt %s[%ld] = {\n", const_class, name, (long) Size);
	if (packoutput)
	{
		sep = ",";
		for (i = 0; i < Size; i++)
		{
			fprintf(fd, "\t%s %ld, %ld, %ld %s%s\n",
					prebrace, (long) T->State, (long) T->Width, (long) T->Param, postbrace, i == (Size - 1) ? "" : ",");
			T++;
		}
	} else
	{
		...
		for (i = 0; i < Size; i++)
		{
			fprintf(fd, "%s%s%3ld,%3ld,%4ld%s",
					sep, prebrace, (long) T->State, (long) T->Width, (long) T->Param, postbrace);
			if (((i + 1) % 6) == 0)
				sep = ",\n ";
			else
				sep = ",";
