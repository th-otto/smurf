...
static int dither_destruktiv(int dest_depth, int dest_dither, int dest_pal)
{
	...
	if (dest_pal == CR_SYSPAL && dest_depth != Sys_info.bitplanes)
	{
		...
		{
			Dialog.winAlert.openAlert("Kein NCT-File fÅr die gewÑhlte Farbtiefe gefunden. Dithering nicht mîglich.",
									  NULL, NULL, NULL, 1);
			return (-1);
		}
	}
...
/*	printf("\n %li Farben", colcount);*/
	...
	else if (colcount <= 32768L)
	{
		if (smurf_picture[active_pic]->depth == 24)
			if (Dialog.winAlert.
				openAlert
				("Das Bild kann nach 16 Bit Farbtiefe gewandelt werden. Folge kann aber ein QualitÑtsverlust sein!",
				 "Abbruch", " OK ", NULL, 1) == 2)
