...
/*	Beim Initialisieren des Plugins mit MSTART wird diesem in seiner
	...
	zu kînnen.
	Die IDs fÅr Exporter beginnen mit MOD_EXPORT, demnach stehen bis zu 255
...
void scan_plugins(void)
{
	...
	while (actual != NULL)
	{
		...
		} else
		{
			...
			/* Zeiger auf Magic (muû 'PLGN' sein!) */
			...
			} else
			{
				...
				{
					/* FIXME: translate */
					sprintf(alert, "[2][Das Plugin \"%s\"|ist nicht fÅr diese Smurf-Version|gedacht! Da das zu "
						"Problemen|fÅhren kann, wird das Plugin terminiert.][ OK ]", curr_info->name);
					form_alert(1, alert);
					install_flag = FALSE;
				} else
				{
