...
WORD f_handle_message(void)
{
	 ...
	 */
	if ((messagebuf[0] >= 20 && messagebuf[0] <= 36) || messagebuf[0] == WM_SHADED || messagebuf[0] == WM_UNSHADED)
	{
		...
		else if (!window_to_handle->pflag)
	...
	case WM_TOPPED:
		...
		else
		{
			...
			} else if (wind_num < 0)	/* Bildfenster */
			{
	...
	case WM_ONTOP:
		if (wind_num < 0)				/* Bildfenster */
		{
			...
			if (Display_Opt.palette_mode == PAL_TOPWIN)
				f_set_picpal(window_to_handle->picture);
		} else if (wind_num >= 0)		/* Dialogfenster */
		{
			f_set_syspal();
			Window.cursorOn(window_to_handle);
			Dialog.picMan.tree[PM_BOX].ob_spec.obspec.framesize = 0;
	...
	case VA_THAT_IZIT:
		...
		if (messagebuf[4] == VA_OB_SHREDDER || messagebuf[4] == VA_OB_TRASHCAN)
		{
			...
			} else if (Comm.avComm.type == AV_BLOCK)	/* Block wurde weggeworfen */
			{
				if (picnum < 0)
					imageWindow.removeBlock(&picture_windows[-picnum]);
			}
		} else if (messagebuf[4] == VA_OB_CLIPBOARD)
		{
	...
	case BUBBLEGEM_REQUEST:
		...
		else if (wind_num == 0 && Window.myModuleWindow(messagebuf[3]))
	...
	case OLE_EXIT:
		 ...
		 */
	case OLE_NEW:
		Sys_info.olgaman_ID = messagebuf[1];
