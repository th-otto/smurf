...
void f_options(void)
{
	...
	if (button == OPT_OK || button == OPT_SET)
	{
		...
		else if (infowindow[ENV_SILLY].ob_state & OS_SELECTED)
			Sys_info.environment = 2;
		else if (infowindow[ENV_THERAPY].ob_state & OS_SELECTED)
			Sys_info.environment = 3;

		if (infowindow[KBEV_JOIN].ob_state & OS_SELECTED)
			Sys_info.keyevents = KBEV_JOIN;
		else if (infowindow[KBEV_DELETE].ob_state & OS_SELECTED)
